...
void UCombatComponent::RotateToTarget(float DeltaTime)
{
	...
	FRotator InterpRotation = FMath::RInterpTo(GetOwner()->GetActorRotation(), LookAtRotationYaw, DeltaTime, 15.f);
...
void UCombatComponent::TakeDamage(float Damage)
{
	...
	if (Random)
	{
		...
		if (AnimInstance && CombatMontage)
		{
			...
			UE_LOG(LogTemp, Warning, TEXT("Anim OK"));
		}
		else
		{
...
void UCombatComponent::ApplyDamage()
{
	...
	int32 nTotalDmg{};
	nTotalDmg = Owner->getUnitStat()->ATK + FMath::RandRange(Owner->getUnitStat()->STR, Owner->getUnitStat()->STR * 10);
...
void UCombatComponent::Attack()
{
	...
	if (Owner->getUnitState() == EUNIT_STATE::E_Attack || 
		m_pTarget->getUnitState() == EUNIT_STATE::E_Dead ||

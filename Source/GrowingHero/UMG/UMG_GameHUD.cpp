...
{
	if (!ensure(m_pMyController->InventorySystemComponent != nullptr))
		return nullptr;

	if (!ensure(m_pMyController->InventorySystemComponent->InventoryFrame != nullptr))
		return nullptr;
...
{
	return m_pMyController->InventorySystemComponent->InventoryFrame->InventoryWindowConsumption;
}

UUMG_InventoryWindow* UUMG_GameHUD::getInventoryWindowEquipment()
{
	return m_pMyController->InventorySystemComponent->InventoryFrame->InventoryWindowEquipment;
}

UUMG_InventoryWindow* UUMG_GameHUD::getInventoryWindowEtc()
{
	return m_pMyController->InventorySystemComponent->InventoryFrame->InventoryWindowEtc;
}

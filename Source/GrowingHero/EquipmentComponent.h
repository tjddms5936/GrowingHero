...
public:
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "EquipmentComponent|Info")
	TArray<FInventoryEquipment> m_arStrEquipItem;

	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "EquipmentComponent|Info")
	TArray<class AEquipItem*> m_arEquipItem;

	void EnterEquipment(EEquipType eType, FInventoryEquipment FItem, AEquipItem* pEquipItem);
	void ReleaseEquipment(EEquipType eType);
